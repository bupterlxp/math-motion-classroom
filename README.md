...
生产环境可以直接部署到 Vercel：导入仓库，添加 `OPENAI_API_KEY`（可选 `OPENAI_MODEL`），Vercel 会自动识别 `api/generate.js`。如果前端仍托管在 GitHub Pages，可以使用 `?api=https://你的-api-域名` 指定后端地址；后端需将 `ALLOWED_ORIGIN` 设置为 Pages 域名。
